...
| A method with a `void`, possibly asynchronous (for example, `Mono<Void>`), return type (or a `null` return
  ...
  // RickyDelete: See <<webflux-caching-etag-lastmodified>> for details.
...
// RickyDelete: See also <<webflux-caching-static-resources, HTTP caching support for static resources>>.
